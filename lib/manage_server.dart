...
class _ManageServerPageState extends State<ManageServerPage> {
  ...
  final Color accentRed = const Color(0xFFFFC107);
  ...
  Future<void> _fetchVpsList() async {
    ...
    final uri = Uri.parse('$baseUrl/vps/myServer?key=${widget.keyToken}');
    try {
  ...
  Future<void> _addVps() async {
    ...
    final uri = Uri.parse('$baseUrl/vps/addServer');
    try {
  ...
  Future<void> _deleteVps(String host) async {
    final uri = Uri.parse('$baseUrl/vps/delServer');
    try {
      final res = await http.post(uri, body: {
        'key': widget.keyToken,
        'host': host,
